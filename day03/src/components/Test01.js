...
const Test01 = () => {
    ...
    const onInputId = (event) =>{
        //input태그 안에 있는 모든 속성들을 접근(type, value, onChange)중에서 value만 가져온다
        const {value}=event.target //이벤트 발생한 대상
        setId(value)
    }
    ...
    return (
        <div>
            아이디 &emsp;<input type='text' value={id} onChange={onInputId} ref={idRef}/>
            <br/><br/>
